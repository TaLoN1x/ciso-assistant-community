<script lang="ts">
	...
	const nextVersion = (data.latest?.version ?? 0) + 1;
	...
	function setAction(idx: number, action: DraftAction) {
		...
		// Open file picker for the replacement
	...
	async function submit() {
		...
		try {
			...
			await goto(`/evidences/${data.evidence.id}/`, { invalidateAll: true });
		} catch (e: any) {
...
<div class="card bg-white shadow-lg p-6 space-y-4">
	<header class="flex items-start justify-between gap-4">
		<div>
			<div class="text-xs uppercase tracking-wide text-gray-500">{'New revision'}</div>
			<h1 class="text-xl font-semibold">{data.evidence.name}</h1>
			<div class="text-sm text-gray-600 mt-1">
				{#if data.latest}
					{'Current'}: <strong>v{data.latest.version}</strong>
					· {inherited.length}
		...
		<a href={`/evidences/${data.evidence.id}/`} class="btn preset-tonal-surface">
			<i class="fa-solid fa-times mr-1"></i>{m.cancel()}
		</a>
	</header>
	...
	{#if !isSimpleCase}
		<section class="border rounded">
			<div class="px-3 py-2 bg-gray-50 border-b text-xs uppercase tracking-wide text-gray-600">
				{'Files in current revision'} — v{data.latest.version}
			</div>
				...
				{#each additions as a, idx}
					<li class="py-1 flex items-center gap-2">
						...
						<button
							...
							onclick={() => (additions = additions.filter((_, i) => i !== idx))}
							>×</button
						>
					</li>
				{/each}
				...
				{#each hashing as h}
					<li class="flex items-center gap-2 text-gray-600">
						...
						{:else if h.status === 'duplicate'}
							...
							<span>{h.name}</span>
							<span class="italic text-gray-500">{'identical to inherited file → nothing to upload'}</span>
						{:else if h.status === 'replaced'}
							<i class="fa-solid fa-arrows-rotate text-amber-600"></i>
							<span>{h.name}</span>
							<span class="italic text-amber-700">{'auto-promoted to Replace (same name, new content)'}</span>
	...
	{#if finalFiles.length > 0}
		<div class="border rounded-lg bg-blue-50 border-blue-200 p-3 text-sm">
			<div class="font-semibold text-blue-900 mb-1">
				<i class="fa-solid fa-eye mr-1"></i>{'Preview of'}
				v{nextVersion}
			</div>
	...
	<div class="flex justify-end gap-2">
		<a href={`/evidences/${data.evidence.id}/`} class="btn preset-tonal-surface">{m.cancel()}</a>
		<button
			type="button"
			disabled={submitting}
